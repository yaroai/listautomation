...
async function doPreview(){
  ...
  try{
    const r=await fetch("/preview",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(opts())});
    if(!r.ok){ const j=await r.json().catch(()=>({error:"preview failed"})); err.textContent=j.error||"preview failed"; }
    else{ const blob=await r.blob(); frame.src=URL.createObjectURL(blob); }
...
previewBtn.onclick=async()=>{
  ...
  try{
    ...
    if(j.error){ err.textContent=j.error; }
      ...
      frame.hidden=true; pvid.hidden=false;
